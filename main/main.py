...
def sub_cb(topic, msg):
    ...
    if topic == SUB_TOPIC_CONFIG:
        ...
        if relay_manager is None:
            relay_manager = RelayManager(client, str_client_id, PUB_TOPIC_CONFIG, relays)
        else:
            relay_manager.add_relays(relays)
    elif topic == SUB_TOPIC_RELAY:
        relay = json.loads(msg.decode('utf-8'))
        if relay_manager:
            relay_manager.update_relay(relay, PUB_TOPIC_RELAY)
    elif topic == SUB_TOPIC_UPDATE:
        update_firmware()
    relay_manager.update_state(PUB_TOPIC_STATUS)
